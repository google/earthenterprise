 ...
 * delegating an actual initialization job to the doInit() by setting it as
...
function geeLoadMap(serverDefs, geeMapOpts) {
  // Create the map.
  geeMap = geeCreateFusionMap(geeDivIds.map, serverDefs, geeMapOpts);
...
function geeCreateLayerItem(serverUrl, layerList, layer) {
  ...
  item.onclick = function(layerObject) {
    return function(e) {
      ...
      geeMap.panAndZoom(lookAt.lat, lookAt.lng, lookAt.zoom);
      cancelEvent(e);
    };
  }(layer);
...
function geePanTo(lat, lng, zoomLevel) {
  geeMap.panAndZoom(lat, lng, zoomLevel);
}
