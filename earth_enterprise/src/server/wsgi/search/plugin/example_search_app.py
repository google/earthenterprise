...
"""Module for Example Search WSGI application."""
from search.common import exceptions
from search.plugin import example_search_handler


class GeExampleSearchApp(object):
  """GeExampleSearchApp implements the callable method.
  ...
  def __init__(self):
    ...
    self._status = "%s"
    self._example = example_search_handler.ExampleSearch()

  def __call__(self, environ, start_response):
    """Callable method for the WSGI application.

    Callable method which performs a example search and returns the
    ...
    try:
      response_body, response_type = self._example.HandleSearchRequest(environ)

      content_type = self._example.utils.GetContentType(response_type)
      ...
      content_type = self._example.utils.content_type % ("text/plain")
      status = self._status % ("500 Internal Server Error")
      self._example.logger.debug(
          "%s, status %s sent back to client."
          % (e, status))
