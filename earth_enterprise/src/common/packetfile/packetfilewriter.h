 ...
 public:
  ...
  class AllocatedBlock {
   ...
   private:
    ...
    uint64 index_pos_;
    size_t data_size_;
    inline AllocatedBlock(uint64 data_pos, uint64 index_pos, size_t data_size) :
  ...
  uint64 WriteAppendCRC(const QuadtreePath &qt_path,
                        void *buffer,
                        size_t buffer_size,
                        uint32 index_extra = 0);
...
class PacketFilePackIndexer {
 public:
  ...
  uint64 ReadNext(void *buffer, size_t read_len);
  ...
  uint64 ReadAtLinear(uint64 linear_pos, void *buffer, size_t read_len);

  bool AtEnd() { return data_pos_ >= data_reader_.data_size(); }
  const std::string &abs_path() const { return data_reader_.abs_path(); }
  uint64 data_size() const { return data_reader_.data_size(); }
 ...
 private:
  uint64 data_pos_;                     // linear address position
