...
class TerrainPacketItem {
 public:
  ...
  inline uint64 position() const { return position_; }
  inline uint32 record_size() const { return record_size_; }
  inline uint32 version() const { return version_; }
  inline uint32 providerId() const { return providerId_; }
 private:
  uint64 position_;
...
class TerrainCombiner {
 ...
 private:
  ...
  class PacketInfo {
  public:
    PacketInfo(QuadtreePath even_path, uint32 provider_id,
               uint progress_increment)
  ...
  uint num_cpus_; // Useful for specifying number of compressor threads.
  bool canceled_;
