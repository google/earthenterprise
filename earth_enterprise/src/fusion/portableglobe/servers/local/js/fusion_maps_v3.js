...
var GEE_EARTH_IMAGE_PATH = GEE_STATIC_URL + '/local/earth/images/';
var GEE_MAPS_IMAGE_PATH = GEE_STATIC_URL + '/local/maps/api/icons/';
 ...
 */
function geeInitMap(geeMapOpts) {
  // Issue asynchronous HTTP request to get the ServerDefs.
  // Set the doInitMap() as a callback-function which will be triggered on
  // finishing the request.
  getServerDefinitionAsync(
      GEE_SERVER_URL, true, makeDoInitMapCallback(geeMapOpts));
}
 ...
 */
function makeDoInitMapCallback(geeMapOpts) {
  function func() {
    doInitMap(geeMapOpts);
  }
  return func;
}
...
function doInitMap(geeMapOpts) {
  ...
  var latLngZoom = getLatLngZoomParameter();
  if (latLngZoom) {
    // Options passed to the v3 map object.
    geeMapOpts = {
          zoom: parseInt(latLngZoom.zoom),
          center: new google.maps.LatLng(parseFloat(latLngZoom.lat),
                                         parseFloat(latLngZoom.lng)),
...
function geeToggleLayer(e, checkBoxId, channel, glmId, layerName) {
  try {
    ...
    try {
      if (cb.checked) {
        geeMap.showFusionLayer(channel, glmId);
      } else {
        geeMap.hideFusionLayer(channel, glmId);
      }
    } catch (err2) {
