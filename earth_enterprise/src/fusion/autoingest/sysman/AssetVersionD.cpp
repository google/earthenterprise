  ...
  for (auto ref : parentsToNotify) {
    ...
    if (assetVersion) {
      ...
      notify(NFY_WARN, "'%s' has broken parent '%s'",
             std::string(assetVersion->GetRef()).c_str(),
             std::string(ref).c_str());
    }
    i++;
  }
  ...
  for (std::vector<AssetVersion>::const_iterator i =
    ...
    if (input) {
      ...
      notify(NFY_WARN, "StateByInputs: %s missing input %s",
             std::string(GetRef()).c_str(),
             std::string(input->GetRef()).c_str());
      ++numblocking;
      // At this point we already know what the values of all of the outputs
      // will be (statebyinputs will be Blocked, blockersAreOffline will
      // be false since numblockers will be different from numoffline, and
...
AssetVersionImplD::HandleChildStateChange(const std::shared_ptr<StateChangeNotifier>) const
{
  ...
  notify(NFY_VERBOSE, "AssetVersionImplD::HandleChildStateChange: %s", std::string(GetRef()).c_str());
}
  ...
  for (const auto &p : parents) {
    ...
    if (parent) {
      if ((parent->state != AssetDefs::Offline) &&
        ...
        if (wouldbreak) {
          wouldbreak->push_back(std::string(p));
        } else {
          return false;
        }
      }
...
{
  notify(NFY_VERBOSE, "HandleInputStateChange: %s", std::string(GetRef()).c_str());
...
LeafAssetVersionImplD::SubmitTask(void)
{
  try {
    ...
    notify(NFY_WARN, "Exception during SubmitTask: %s : %s",
           std::string(GetRef()).c_str(), e.what());
...
{
  notify(NFY_VERBOSE, "CompositeAssetVersionImplD::HandleChildStateChange: %s", std::string(GetRef()).c_str());
  SyncState(notifier);
}
