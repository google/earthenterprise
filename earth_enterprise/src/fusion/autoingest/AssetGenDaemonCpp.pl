...
{
    $calcParam=", calc";
    $finalArg=", std::shared_ptr<OverlapCalculator<$templateName>> calc";
}
print $fh <<EOF;
        ...
        $finalArg)
{
    ...
    bool needed = false;
    return asset->MyUpdate(needed $forwardcachedinputarg
                           $forwardExtraUpdateArg $calcParam);
}
...
if ($base eq 'Composite') {
    print $fh "        newver->UpdateChildren($singleForwardExtraUpdateArg $calcParam);\n";
}
...
$template
${name}AssetVersionD
${name}AssetImplD::MyUpdate(bool &needed $formalExtraUpdateArg $finalArg) const
{
EOF
