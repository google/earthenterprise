...
public:
    ...
    */
    static U16CPU ComputeRowBytes(Config c, U16CPU width);

    /** Set the bitmap's config and dimensions. If rowBytes is 0, then ComputeRowBytes() is
        called to compute the optimal value. If getOwnsPixels() returned true,
...
//  void    resizeAlloc(U16CPU width, U16CPU height);
    ...
    */
    ...
    /** Assign ctable to be the colortable for the bitmap, replacing any existing reference.
        The reference count of ctable (if it is not null) is incremented, and any existing
    ...
    */
    ...
    void extractAlpha(SkBitmap* dst) const { this->extractAlpha(dst, NULL, NULL); }
    void extractAlpha(SkBitmap* dst, const SkPaint* paint, SkPoint16* offset) const;
