...
public:
    ...
    virtual bool setContext(const SkBitmap& device, const SkPaint& paint, const SkMatrix& matrix)
    {
        ...
        // in case we're a mipmap, we perform this here, after calling inherited::setContext
        // so that srcBitmap will be correct.
        const SkBitmap& srcBitmap = this->getSrcBitmap();
        fPreRoundX = SK_FixedHalf / srcBitmap.width();
        fPreRoundY = SK_FixedHalf / srcBitmap.height();
#else
        fPreRoundX = SK_FixedHalf;
        fPreRoundY = SK_FixedHalf;
#endif
        return true;
    }
    ...
    virtual void shadeSpan(int x, int y, SkPMColor dstC[], int count)
    {
        ...
        if (this->getInverseClass() == kPerspective_MatrixClass)
        {
            ...
            while ((count = iter.next()) != 0)
            {
                ...
                SkFixed        preRoundX = fPreRoundX;
                SkFixed        preRoundY = fPreRoundY;
...
*/
        ...
        // now init fx, fy, dx, dy
        {
            ...
            fx = SkScalarToFixed(srcPt.fX) + fPreRoundX;
            fy = SkScalarToFixed(srcPt.fY) + fPreRoundY;
