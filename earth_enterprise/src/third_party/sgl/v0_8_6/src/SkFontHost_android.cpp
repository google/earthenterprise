...
enum {
    ...
    Ahem_FAMILY_INDEX,  // test font for web browser
...
static const FontFaceRec gSerifFaces[] = {
    { "DroidSerif-Regular.ttf",     SERIF_FAMILY_INDEX, 0,  0 },
    { "DroidSerif-Bold.ttf",        SERIF_FAMILY_INDEX, 1,  0 },
    { "DroidSerif-Italic.ttf",      SERIF_FAMILY_INDEX, 0,  1 }
    // bolditalic is on its way
};

static const FontFaceRec gMonoFaces[] = {
    { "DroidSansMono.ttf", MONO_FAMILY_INDEX, 0,  0 }
};
...
static const FontFaceRec gAhemFaces[] = {
    { "Ahem.ttf", Ahem_FAMILY_INDEX, 0,  0 }
};
...
static const FontFamilyRec gFamilies[] = {
    ...
    { gAhemFaces,   SK_ARRAY_COUNT(gAhemFaces)  }    // test font for web browser
};
...
{
    const FontFaceRec** face = (const FontFaceRec**)desc->findEntry(kTypeface_SkDescriptorTag, NULL);
    // we build a complete filename for our key string
    key->set(getenv("ANDROID_ROOT"));
    key->append(kFontFilePrefix);
    key->append((*face)->fFileName);
}
