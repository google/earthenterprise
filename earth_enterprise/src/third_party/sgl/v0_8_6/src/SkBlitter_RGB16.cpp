...
void SkRGB16_Black_Blitter::blitAntiH(int x, int y, const SkAlpha antialias[], const int16_t runs[])
{
...
// experiment in dithering
#define DITHER_SOLID_COLOR

#ifdef DITHER_SOLID_COLOR
static void sk_dither_memset16(uint16_t dst[], uint16_t value, uint16_t other, int count)
{
    if (count > 0)
    {
...
SkRGB16_Blitter::SkRGB16_Blitter(const SkBitmap& device, const SkPaint& paint) : fDevice(device)
{
    ...
    fRawColor16 = SkPack888ToRGB16(r, g, b);
#ifdef DITHER_SOLID_COLOR
...
void SkRGB16_Blitter::blitAntiH(int x, int y, const SkAlpha antialias[], const int16_t runs[])
{
    if (fScale == 0)
        return;
    ...
    if (scale == 256)
    {
#ifdef DITHER_SOLID_COLOR
        uint16_t    ditherColor = fRawDither16;
        uint16_t    baseColor = srcColor;
        if ((x ^ y) & 1)
            SkTSwap(ditherColor, baseColor);
#endif
        ...
        for (;;)
        {
            ...
            if (aa)
            {
                ...
                {
#ifdef DITHER_SOLID_COLOR
                    sk_dither_memset16(device, baseColor, ditherColor, count);
#else
    ...
    if (fScale == 256)
    {
        do {
            ...
            do {
                ...
                unsigned dst_scale = SkAlpha255To256(255 - aa);
                *device = (uint16_t)RGB16Add(SkAlphaMulRGB16(color16, src_scale), SkAlphaMulRGB16(*device, dst_scale));
                device += 1;
            } while (--w != 0);
            device = (uint16_t*)((char*)device + deviceRB);
            alpha += maskRB;
...
void SkRGB16_Shader_Xfermode_Blitter::blitAntiH(int x, int y, const SkAlpha antialias[], const int16_t runs[])
{
    SkShader*   shader = fShader;
