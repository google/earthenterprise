...
{
    if (fMat[SkMatrix::kMPersp0] || fMat[SkMatrix::kMPersp1] || fMat[SkMatrix::kMPersp2] != kMatrix22Elem)
        return false;   // perspective does not preserve rectness

    // record if the given element is non-zero (answer is 0 or 1)
    int m00 = fMat[SkMatrix::kMScaleX] != 0;
    int m01 = fMat[SkMatrix::kMSkewX] != 0;
    int m10 = fMat[SkMatrix::kMSkewY] != 0;
    int m11 = fMat[SkMatrix::kMScaleY] != 0;

    // record if the (p)rimary and (s)econdary diagonals are all 0 or all non-zero (answer is 0 or 1)
    ...
    // return 1 if primary is 1 and secondary is 0 or primary is 0 and secondary is 1
    return (dp0 & ds1) | (dp1 & ds0);
}
...
////////////////////////////////////////////////////////////////////////////////////
    ...
    static void set_muladdmul(Sk64* dst, int32_t a, int32_t b, int32_t c, int32_t d)
    {
        Sk64    tmp;
...
void SkMatrix::identity_MapProc(SkPoint dst[], const SkPoint src[], int count) const
{
    SkASSERT(this->getType() == 0);

    if (dst != src && count > 0)
        memcpy(dst, src, count * sizeof(SkPoint));
}

void SkMatrix::trans_MapProc(SkPoint dst[], const SkPoint src[], int count) const
{
    SkASSERT(this->getType() == kTranslate_Mask);
...
void SkMatrix::scale_MapProc(SkPoint dst[], const SkPoint src[], int count) const
{
    SkASSERT(this->getType() == kScale_Mask);
...
void SkMatrix::scaleTrans_MapProc(SkPoint dst[], const SkPoint src[], int count) const
{
    SkASSERT(this->getType() == (kScale_Mask | kTranslate_Mask));
...
void SkMatrix::rot_MapProc(SkPoint dst[], const SkPoint src[], int count) const
{
    SkASSERT((this->getType() & (kPerspective_Mask | kTranslate_Mask)) == 0);

    if (count > 0)
    {
        SkScalar mx = fMat[kMScaleX];
        SkScalar my = fMat[kMScaleY];
        SkScalar kx = fMat[kMSkewX];
        SkScalar ky = fMat[kMSkewY];
        do {
            SkScalar sy = src->fY;
            SkScalar sx = src->fX;
            src += 1;
...
void SkMatrix::rotTrans_MapProc(SkPoint dst[], const SkPoint src[], int count) const
{
    SkASSERT((this->getType() & kPerspective_Mask) == 0);
...
void SkMatrix::persp_MapProc(SkPoint dst[], const SkPoint src[], int count) const
{
    SkASSERT(this->getType() & kPerspective_Mask);
...
#else
            float z = SkScalarMul(sx, fMat[kMPersp0]) + SkScalarMulAdd(sy, fMat[kMPersp1], fMat[kMPersp2]);
#endif
            if (z)
...
const SkMatrix::MapProc SkMatrix::gMapPocs[] = {
    &SkMatrix::identity_MapProc,
    &SkMatrix::trans_MapProc,
    &SkMatrix::scale_MapProc,
    &SkMatrix::scaleTrans_MapProc,
    &SkMatrix::rot_MapProc,
    &SkMatrix::rotTrans_MapProc,
    &SkMatrix::rot_MapProc,
    &SkMatrix::rotTrans_MapProc
};

SkMatrix::MapProc SkMatrix::GetMapProc(TypeMask typeMask)
{
    SkASSERT((size_t)(typeMask & ~kPerspective_Mask) < SK_ARRAY_COUNT(SkMatrix::gMapPocs));

    SkMatrix::MapProc proc;

    if (typeMask & kPerspective_Mask)
        proc = &SkMatrix::persp_MapProc;
    else
        proc = SkMatrix::gMapPocs[typeMask];
    return proc;
}
...
bool SkMatrix::mapPoints(SkPoint dst[], const SkPoint src[], int count, TypeMask typeMask) const
{
    ...
    SkASSERT(src == dst || SkAbs32((int32_t)(src - dst)) >= count); // no partial overlap

    SkMatrix::MapProc proc = SkMatrix::GetMapProc(typeMask);
    (this->*proc)(dst, src, count);
    return true;
}
...
bool SkMatrix::mapVectors(SkPoint dst[], const SkPoint src[], int count, TypeMask maskType) const
{
    ...
    else
    {
        ...
        ok = tmp.mapPoints(dst, src, count, TypeMask(maskType & ~kTranslate_Mask));
    }
    return ok;
}
...
#else
    pt->fX = SkScalarMul(sx, m.fMat[kMScaleX]) + SkScalarMulAdd(sy, m.fMat[kMSkewX], m.fMat[kMTransX]);
    pt->fY = SkScalarMul(sx, m.fMat[kMSkewY]) + SkScalarMulAdd(sy, m.fMat[kMScaleY], m.fMat[kMTransY]);
#endif
}
...
void SkMatrix::dump() const
{
    ...
    SkDebugf("[%8.4f %8.4f %8.4f] [%8.4f %8.4f %8.4f] [%8.4f %8.4f %8.4f]\n",
#ifdef SK_SCALAR_IS_FLOAT
...
void SkMatrix::UnitTest()
{
    ...
    // rectStaysRect test
    {
        ...
        for (size_t i = 0; i < SK_ARRAY_COUNT(gRectStaysRectSamples); i++)
        {
            ...
            m.reset();
            m[0] = gRectStaysRectSamples[i].m00;
            m[1] = gRectStaysRectSamples[i].m01;
            m[3] = gRectStaysRectSamples[i].m10;
            m[4] = gRectStaysRectSamples[i].m11;
