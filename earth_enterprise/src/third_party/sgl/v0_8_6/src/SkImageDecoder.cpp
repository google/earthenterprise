...
SkImageDecoder::SkImageDecoder() : fPeeker(NULL), fChooser(NULL), fAllocator(NULL)
{
}
...
SkImageDecoder::Allocator* SkImageDecoder::setAllocator(Allocator* allocator)
{
...
bool SkImageDecoder::allocPixels(SkBitmap* bitmap) const
{
    Allocator* allocator = fAllocator;

    if (allocator)
        return allocator->allocPixels(bitmap);
...
SkBitmapRef* SkBitmapRef::DecodeMemory(const void* bytes, size_t len, const char* name)
{
    ...
    SkAutoTDelete<Rec>  autoRec(rec = SkNEW_ARGS(Rec, (name != NULL)));
