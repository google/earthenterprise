...
/*
    Need to handle the case when the angle is sharp, and our computed end-points for the arc go behind
    pt1 and/or p2...
*/
void SkPath::arcTo(SkScalar x1, SkScalar y1, SkScalar x2, SkScalar y2, SkScalar radius)
{
    ...
    // need to know our prev pt so we can construct tangent vectors
    {
        ...
        before.setUnit(x1 - start.fX, y1 - start.fY);
        after.setUnit(x2 - x1, y2 - y1);
    }
    ...
    matrix.postTranslate(xx - SkScalarMul(radius, before.fX), yy - SkScalarMul(radius, before.fY));
...
bool SkPath::transform(const SkMatrix& matrix, SkPath* dst) const
{
    ...
    else
    {
        ...
        if (!fFastBoundsIsDirty && matrix.rectStaysRect())
        {
...
uint32_t SkPath::flatten(void* storage) const
{
    if (storage)
    {
