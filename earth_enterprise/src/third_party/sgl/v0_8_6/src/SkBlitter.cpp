...
void SkRectClipBlitter::blitMask(const SkMask& mask, const SkRect16& clip)
{
    SkASSERT(mask.fBounds.contains(clip));
...
void SkRgnClipBlitter::blitAntiH(int x, int y, const SkAlpha aa[], const int16_t runs[])
{
    ...
    SkRegion::Spanerator span(*fRgn, y, x, x + width);
    int left, right, lastIndex = 0, offset = 0;
    bool atLeastOneSpan = false;
    ...
    while (span.next(&left, &right))
    {
        ...
        if (!atLeastOneSpan && x < left)
        {
            ...
            offset = left - x;  // so we know how much to skip before calling the real blitter
        }
        atLeastOneSpan = true;
        ...
        lastIndex = right - x;
        ((uint8_t*)aa)[lastIndex] = 0;   // skip runs after right
        ((int16_t*)runs)[lastIndex] = SkToS16(right - left);
    }
    
    if (atLeastOneSpan)
    {
        ((int16_t*)runs)[lastIndex] = 0;
        fBlitter->blitAntiH(x + offset, y, aa + offset, runs + offset);
    }
}
...
void SkRgnClipBlitter::blitMask(const SkMask& mask, const SkRect16& clip)
{
    SkASSERT(mask.fBounds.contains(clip));
