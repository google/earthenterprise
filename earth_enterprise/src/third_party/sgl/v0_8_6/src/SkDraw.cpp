...
{
    fDevice     = &canvas.getCurrDevice().accessBitmap();
...
void SkDraw::drawBitmapAsMask(const SkBitmap& bitmap, SkScalar x, SkScalar y, const SkPaint& paint)
{
    ...
    if (just_translate(*fMatrix))
    {
        ...
        pt.set(x, y);
        fMatrix->mapPoints(&pt, &pt, 1);
        
        int     ix = SkScalarRound(pt.fX);
        int     iy = SkScalarRound(pt.fY);

        SkMask  mask;
        mask.fBounds.set(ix, iy, ix + bitmap.width(), iy + bitmap.height());
        mask.fFormat = SkMask::kA8_Format;
        mask.fRowBytes = bitmap.rowBytes();
        mask.fImage = (uint8_t*)bitmap.getPixels();
    ...
    else    // need to xform the bitmap first
    {
        ...
        r.set(x, y, x + SkIntToScalar(bitmap.width()), y + SkIntToScalar(bitmap.height()));
        fMatrix->mapRect(&r);
        r.round(&mask.fBounds);
        
        // set the mask's bounds to the transformed bitmap-bounds, clipped to the actual device
        {
            SkRect16    devBounds;
            devBounds.set(0, 0, fDevice->width(), fDevice->height());
            // need intersect(l, t, r, b) on rect16
...
void SkDraw::drawBitmap(const SkBitmap& bitmap, SkScalar x, SkScalar y, const SkPaint& paint)
{
    ...
    if (bitmap.getConfig() == SkBitmap::kA8_Config && paint.getMaskFilter() != NULL)
    {
        this->drawBitmapAsMask(bitmap, x, y, paint);
        return;
    }
...
static void handle_aftertext(SkDraw* draw, const SkPaint& paint, SkScalar width, const SkPoint& start)
{
    ...
    if (flags & (SkPaint::kUnderlineText_Flag | SkPaint::kStrikeThruText_Flag))
    {
        ...
        {
            SkScalar offset = SkScalarMulAdd(textSize, kStdUnderline_Offset, start.fY);
            r.fTop = offset;
        ...
        {
            SkScalar offset = SkScalarMulAdd(textSize, kStdStrikeThru_Offset, start.fY);
            r.fTop = offset;
            r.fBottom = offset + height;
            draw_paint_rect(draw, paint, r, textSize);
        }
...
#ifdef SHOW_POINT_SIZE
    printf("drawText ----- pointSize %g length %d\n", SkScalarToFloat(paint.getTextSize()), byteLength);
#endif
...
#ifdef SHOW_POINT_SIZE
    printf("drawPosText ----- pointSize %g length %d\n", SkScalarToFloat(paint.getTextSize()), byteLength);
#endif
...
{
    SkMatrix::MapPtProc proc = matrix.getMapPtProc();
