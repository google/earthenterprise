...
java_print_version_numeric()
{
    local RESULT=$(java_print_property "java.version")

    # Strip any non-numeric part of the version:
    RESULT="${RESULT%%[!0-9.]*}"
...
if [ "$PACKAGE_MANAGER" == "yum" ]; then
    ...
    install_packages()
    {
        ...
        JRE_VERSION=$(java_print_version_numeric)
        if [ -z "$JRE_VERSION" ] || \
        ! is_version_ge "$JRE_VERSION" "$MIN_JRE_VERSION"; then
            yum $ASSUME_YES_PACKAGE_MANAGER_PARAMETER install \
                java-1.8.0-openjdk-headless || return 1
        fi
        yum install $ASSUME_YES_PACKAGE_MANAGER_PARAMETER \
    ...
    install_packages()
    {
        ...
        #   The JRE is needed to run Gradle for building packages.
        ...
        if [ -z "$JRE_VERSION" ] || \
        ! is_version_ge "$JRE_VERSION" "$MIN_JRE_VERSION"; then
            apt-get $ASSUME_YES_PACKAGE_MANAGER_PARAMETER install openjdk-9-jre \
                || return 1
        fi
