...
.. container::
   ...
   .. container:: content
      ...
      Included in ``/gevol/assets/.config/misc.xml`` is a
      ``UseMinification`` option that controls how data for imagery and terrain
      zoom levels is generated.
      ``UseMinification`` can be set to the following values:
      - ``1``: Imagery and terrain for each zoom level is generated by
        minifying the level below it.  This is the default.
      - ``0``: Imagery and terrain for each zoom level is generated from the
        original source data.  This reduces dependencies between levels and
        may improve performance by increasing parallelism in some situations.
